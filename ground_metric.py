...
class GroundMetric:
    ...
    def _clip(self, ground_metric_matrix):
        if self.params.debug:
            logger.info("before clipping {}".format(ground_metric_matrix.data))

        percent_clipped = (
            float((ground_metric_matrix >= self.reg * self.params.clip_max).long().sum().data)
            / ground_metric_matrix.numel()
        ) * 100
        logger.info("percent_clipped is (assumes clip_min = 0) %f", percent_clipped)
        setattr(self.params, "percent_clipped", percent_clipped)
        # will keep the M' = M/reg in range clip_min and clip_max
        ground_metric_matrix.clamp_(min=self.reg * self.params.clip_min, max=self.reg * self.params.clip_max)
        if self.params.debug:
            logger.info("after clipping {}".format(ground_metric_matrix.data))
    ...
    def _normalize(self, ground_metric_matrix):
        if self.ground_metric_normalize == "log":
            ...
            logger.info("Normalizing by max of ground metric and which is {}".format(ground_metric_matrix.max()))
            ground_metric_matrix = ground_metric_matrix / ground_metric_matrix.max()
        elif self.ground_metric_normalize == "median":
            logger.info("Normalizing by median of ground metric and which is {}".format(ground_metric_matrix.median()))
            ground_metric_matrix = ground_metric_matrix / ground_metric_matrix.median()
        elif self.ground_metric_normalize == "mean":
            logger.info("Normalizing by mean of ground metric and which is {}".format(ground_metric_matrix.mean()))
    ...
    def _cost_matrix_xy(self, x, y, p=2, squared=True):
        ...
        if not squared:
            logger.info("dont leave off the squaring of the ground metric")
            c = c ** (1 / 2)
    ...
    def _pairwise_distances(self, x, y=None, squared=True):
        ...
        if self.params.activation_histograms and self.params.dist_normalize:
            ...
            logger.info("Divide squared distances by the num samples")
    ...
    def process(self, coordinates, other_coordinates=None):
        logger.info("Processing the coordinates to form ground_metric")
        if self.params.geom_ensemble_type == "wts" and self.params.normalize_wts:
            logger.info("In weight mode: normalizing weights to unit norm")
        ...
        if self.params.debug:
            logger.info("coordinates is {}".format(coordinates))
            if other_coordinates is not None:
                logger.info("other_coordinates is {}".format(other_coordinates))
            logger.info("ground_metric_matrix is {}".format(ground_metric_matrix))
        ...
        if self.params.debug:
            logger.info("ground_metric_matrix at the end is {}".format(ground_metric_matrix))
