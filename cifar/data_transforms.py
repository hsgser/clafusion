...
from albumentations import Compose, HorizontalFlip, Normalize, PadIfNeeded, RandomCrop, RGBShift, Rotate
...
def albumentations_transforms(p=1.0, is_train=False):
    ...
    transforms_list.extend([Normalize(mean=mean, std=std, max_pixel_value=255.0, p=1.0), ToTensor()])
...
def torch_transforms(is_train=False):
    ...
    if is_train:
        transforms_list.extend([transforms.RandomErasing(0.25)])
