...
do
    echo "ALPHA = $ALPHA"
    python distillation_big_only.py --gpu-id 0 --model-name vgg13_doub_nobias --second-model-name vgg11_half_nobias --n-epochs 300 --save-result-file distill_vgg_small_temp${TEMPERATURE}.csv --exact --correction --ground-metric euclidean --weight-stats --activation-histograms --activation-mode raw --geom-ensemble-type acts --act-num-samples $N_SAMPLES --ground-metric-normalize none --activation-seed 21 --prelu-acts --recheck-cifar --load-models $DATA_PATH --ckpt-type best --dataset cifar10 --past-correction --not-squared --dist-normalize --print-distances --layer-measure activation --layer-metric cka --retrain $RETRAIN --retrain-lr-decay $DECAY --retrain-lr-decay-factor $DECAY_FACTOR --retrain-lr-decay-epochs $DECAY_EPOCHS --alpha $ALPHA --temperature $TEMPERATURE --not-dist-geometric
done